	...
	for _, sub := range processed {
		...
		for i, f := range sub.Submission.Files {
			...
			if f.File.MimeType == echo.MIMEApplicationJSON || f.File.MimeType == echo.MIMETextPlain || f.File.MimeType == MIMETextRTF {
				...
				backupURL := fmt.Sprintf("%s/%s", host, strings.TrimLeft(u.Path, "/"))
...
func CreateTicketReport(auditor *db.Auditor, details []Detail, host *url.URL) TicketReport {
	...
	for i, image := range info.Files {
		if i == 0 {
			message.WriteString(fmt.Sprintf("\n\n[u]MD5 Checksums at the time of writing[/u] ([url=https://inkbunny.net/submissionsviewall.php?text=%s&md5=yes&mode=search]search all[/url]):", strings.Join(info.MD5, "+")))
		}
